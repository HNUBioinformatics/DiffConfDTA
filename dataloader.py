...
def create_DTA_dataset(dataset='davis'):
    ...
    train_drugs, train_drugs_smiles, train_prot_keys, train_prot_seq, train_Y = list(
        df_train_set['compound_iso_smiles']), list(df_train_set['drug_smiles']), list(
        df_train_set['target_key']), list(df_train_set['target_sequence']), list(df_train_set['affinity'])
    train_proteins_vec = [seq_cat(t) for t in train_prot_seq]
    train_smiles_vec = [smi_cat(t) for t in train_drugs_smiles]
    train_drugs, train_prot_keys, train_proteins_vec, train_smiles_vec, train_Y = np.asarray(train_drugs), np.asarray(
        train_prot_keys), np.asarray(train_proteins_vec), np.asarray(train_smiles_vec), np.asarray(train_Y)
    train_dataset = DTADataset(root='data', dataset=dataset + '_' + 'train', xd=train_drugs,
                               ...
                               target_graph=target_graph, smiles_vec=train_smiles_vec, proteins_vec=train_proteins_vec)

    # test dataset construct
    test_csv = dataset + '_test.csv'
    df_test_fold = pd.read_csv(test_csv)
    test_drugs, test_drugs_smiles, test_prots_keys, test_prot_seq, test_Y = list(
        df_test_fold['compound_iso_smiles']), list(df_test_fold['drug_smiles']), list(
    ...
    test_drugs, test_prots_keys, test_proteins_vec, test_smiles_vec, test_Y = np.asarray(test_drugs), np.asarray(
        test_prots_keys), np.asarray(test_proteins_vec), np.asarray(test_smiles_vec), np.asarray(
        test_Y)
    test_dataset = DTADataset(root='data', dataset=dataset + '_' + 'test', xd=test_drugs,
                              ...
                              target_graph=target_graph, smiles_vec=test_smiles_vec, proteins_vec=test_proteins_vec)
